...
export class PhotoViewerV2Component implements OnInit {
  ...
  addFavorite(){
    ...
    if(userId){
      this.userService.addFavoritePhoto(userId, this.photo).subscribe((res:any)=>{
        if(res.status === 'OK'){
          this.isFavorite = true;
          this.notifierService.notify("success", "Foto adicionada aos favoritos!");
        }else{
          this.notifierService.notify("error", "Não foi possível concluir a ação de momento.");
        }
      });
    }
  }
  ...
  removeFavorite(){
    const userId = localStorage.getItem("myPhotos_id");
    if(userId){
      this.userService.removeFavoritePhoto(userId, this.photo).subscribe((res:any)=>{
        if(res.status === 'OK'){
          this.isFavorite = false;
          this.notifierService.notify("success", "Foto removida dos favoritos!");
        }else{
          this.notifierService.notify("error", "Não foi possível concluir a ação de momento.");
        }
      });
  ...
  delete(): void {
    this.photoService.deletePhoto(this.photo).subscribe(
      (res:any)=>{
        if(res.status === 'OK'){
          this.ngOnInit();
          this.notifierService.notify("success", "Foto removida!");
        }else{
          this.notifierService.notify("error", "Não foi possível concluir a ação de momento.");
        }
      }
